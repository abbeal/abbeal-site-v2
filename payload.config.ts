 ...
 *
 * NB : la collection Articles est inlinee ICI (et non importee depuis
 * ./payload/collections/Articles.ts) parce que tsx + tsconfig
 * `moduleResolution: bundler` ne resout pas `./...js` -> `./...ts`
 * automatiquement quand Payload charge ce fichier via CLI. Inliner =
 * zero resolution cross-file = pas de surprise. On reextraira quand
 * on aura plus de 3 collections.
 */
...
// ---------------------------------------------------------------------------
// Collection Articles — miroir du type `Article` de lib/articles.ts.
// 11 types de blocks pour matcher 1:1 l'union ArticleBlock du rendu Next.
// ---------------------------------------------------------------------------
